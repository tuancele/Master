/*
 * File: js/popup.js
 * (Đã cập nhật để gửi Nonce bảo mật)
 */
...
/**
 * Hàm Submit Form chính (cho modal, sidebar, v.v.)
 * Đã cập nhật để gửi `nonce`
 */
function Submit_Form(x, y) {
    ...
    // Lấy nonce từ biến 'formvar' đã được khai báo (từ file misc.php)
    var nonce = formvar.nonce;

    switch (y) {
        case 'noname':
            if (phone1 == "") {
                var phone = !1;
                jQuery(p + " #err_note").css({ display: "block" }), void ErrForm(error2, p)
            } else {
                if (validatePhone(phone1)) {
                    ...
                    var phone = !1;
                    jQuery(p + " #err_note").css({ display: "block" }), void ErrForm(error5, p)
                }
            }

            if (email1 == "") {
                var email = !1;
                jQuery(e + " #err_note").css({ display: "block" }), void ErrForm(error3, e)
            } else {
                if (validateEmail(email1)) {
                    ...
                    var email = !1;
                    jQuery(e + " #err_note").css({ display: "block" }), void ErrForm(error4, e)
                }
            }
            ...
            if (email == !0 && phone == !0) {
                jQuery.ajax({
                    type: 'POST',
                    dataType: 'json',
                    url: ajaxurl, // ajaxurl được định nghĩa bởi WordPress
                    data: {
                        'action': 'cele_ajax',
                        'order_name': 'Noname',
                        'order_phone': phone1,
                        ...
                        'nonce': nonce // <-- THÊM NONCE VÀO ĐÂY
                    },
                    beforeSend: function () {
                    ...
                    error: function (errorThrown) {
                        ...
                        jQuery('.overlay').removeClass('active');
                        alert('Đã xảy ra lỗi. Vui lòng thử lại.'); // Thêm thông báo lỗi
                    }
                });
            }
            break;
        ...
        case 'all':
            if (name1 == "") {
                var name = !1;
                jQuery(n + " #err_note").css({ display: "block" }), void ErrForm(error1, n)
            } else {
                var name = !0;
...
/**
 * Hàm Submit Form trong nội dung (Shortcode [sdt])
 * Đã cập nhật để gửi `nonce`
 */
...
request_content = !1,
    ...
    function (e) {
        e(document).ready(function () {
            ...
            // Logic Affix cho sidebar
            if (e(".section2-left").length && e(".section2-right").length) {
                if (e(".section2-left").height() >= e(".section2-right").height()) {
                    e("#dowload-last-link").affix({
                        offset: {
                            top: e(".section2-right").outerHeight(!0),
                            bottom: e(".section3").outerHeight(!0) + 900 // Cân nhắc điều chỉnh giá trị này
                        }
                    });
                }
            }
            ...
            if (typeof e.fn.mmenu == 'function') {
                e("#menu").mmenu({
                    ...
                    offCanvas: {
                        pageSelector: ".wrappers" // Đảm bảo class này tồn tại
                    }
                });
            }

            // Set width cho sidebar link
            if (e("#dowload-last-link").parent().length) {
        ...
        e(document).ready(function () {
            ...
            // Scroll-to-hash
            e(".tai-bang-gia a, .v2_danhgia_nsb_link a").on("click", function (r) {
                var targetHref = e(this).attr("href");
                // Kiểm tra nếu href là một hash
                if (targetHref && targetHref.startsWith("#")) {
                    var t = e(targetHref);
                    if (t.length > 0) {
                        var offsetTop = t.offset().top - (e(this).hasClass('v2_danhgia_nsb_link') ? 100 : 50);

                            ...
                            <div class="swiper-slide">
                                <div class="card card-subdivision">
                                    ...
                                    <div class="card-block">
                                        ...
                                        <p class="card-value"><?php echo esc_html(get_field('gbdam') ?: 'Đang cập nhật'); ?></p>
                                    </div>
                                    <div class="card-block">
                                        <p class="card-text">Diện tích</p>
                                        <p class="card-value"><?php echo esc_html(get_field('tidam') ?: 'Đang cập nhật'); ?></p>
                                    </div>
                                    <div class="card-block is-price">
                                        <p class="card-text">Tình trạng</p>
                                        <p class="card-value"><?php echo esc_html(get_field('ttdam') ?: 'Đang cập nhật'); ?></p>
                                    </div>
                                    <div class="card-block is-buttons">
                                        <a href="<?php the_permalink(); ?>" class="button is-small is-secondary is-fullwidth" role="link" aria-label="Xem chi tiết">Xem chi tiết</a>
                                    </div>
...
function shortcode_vi_tri_location($atts) {
    ...
    if (!$my_posts) return '';
    $post_ids = array_map(function($p) { return is_object($p) ? $p->ID : $p; }, $my_posts);
    $used_term_ids = [];
                                ...
                                while ($query->have_posts()) : $query->the_post();
                                    ...
                                    <a href="#" class="location-type-lists-item" data-map-src="<?php echo esc_attr($ban_do_src); ?>">
                                        <div class="location-type-start">
                                            ...
                                            <p class="is-body3"><?php echo esc_html(get_field('dia_chi') ?: 'Đang cập nhật'); ?></p>
                                        </div>
                                        <div class="location-type-end is-flex is-align-items-center is-justify-content-flex-end">
                                            ...
                                            <div class="ml-2 location-text">
                                                <p class="is-subtitle1"><?php echo esc_html(get_field('khoang_cach') ?: 'Đang cập nhật'); ?></p>
                                                <p class="is-body3"><?php echo esc_html(get_field('số_phut') ?: 'Đang cập nhật'); ?></p>
                                            </div>
                                        </div>
                                    </a>
                                <?php endwhile; ?>
                ...
                <div class="<?php echo esc_attr($classes); ?>" style="<?php echo esc_attr($style); ?>">
                    ...
                    <div class="accordion-content">
                        <ul>
                            ...
                            <li>
                                <p class="image">
                                    <a data-fancybox="amenity-places" data-caption="<?php the_title(); ?>" href="<?php echo esc_url(get_the_post_thumbnail_url(get_the_ID(), 'large')); ?>">
                                        <?php if (has_post_thumbnail()) {
                                            $post_title = get_the_title();
                                            the_post_thumbnail('full', array('alt' => esc_attr($post_title)));
                                        } ?>
...
function create_shortcode_faq($args, $content) {
    ...
    if (array_key_exists($id, $rows)) {
        $rowhienthi = $rows[$id];
        $row1 = $rowhienthi['tab'];
        $faq = '<div class="accordion"><ul class="accordion__list">';
        if ($row1) {
            $i = 1;
            foreach ($row1 as $row) {
                $faq .= '<li class="accordion__item' . ($i == 1 ? ' active' : '') . '"><div class="accordion__itemTitleWrap"><h3 class="accordion__itemTitle"><a href="#' . $id . $i . 'a"><span>' . esc_html($row['name']) . '</span></a></h3><div class="accordion__itemIconWrap"><svg viewBox="0 0 24 24"><polyline fill="none" points="21,8.5 12,17.5 3,8.5 " stroke="#000" stroke-miterlimit="10" stroke-width="2"/></svg></div></div><div class="accordion__itemContent">' . wp_kses_post($row['content']) . '</div></li>';
                $i++;
            }
            $faq .= '</ul></div>';
        }
...
function create_shortcode_tab($args, $content) {
    ...
    if (array_key_exists($id, $rows)) {
        $rowhienthi = $rows[$id];
        $row1 = $rowhienthi['tab'];
        $tabs = '<div class="tab-shortcode clearfix panel with-nav-tabs"><div class="panel-heading"><ul class="nav nav-tabs">';
        if ($row1) {
            $i = 1;
            foreach ($row1 as $row) {
                $tabs .= '<li class="nav-item' . ($i == 1 ? ' active' : '') . '"><a href="#' . $id . $i . 'a" data-toggle="tab"><span>' . esc_html($row['title']) . '</span></a></li>';
                $i++;
            }
            $tabs .= '</ul></div><div class="panel-body"><div class="tab-content">';
            $i = 1;
            foreach ($row1 as $row) {
                $tabs .= '<div class="tab-pane' . ($i == 1 ? ' active' : '') . '" id="' . $id . $i . 'a"><img src="' . esc_url($row['img']) . '" class="img-responsive" alt="banner"/><div class="content">' . wp_kses_post($row['content']) . '</div></div>';
                $i++;
            }
            $tabs .= '</div></div></div>';
        }
        if (cele_is_amp()) {
            $tabs = '<div class="tab-shortcode"><amp-selector class="tabs-with-selector" id="carouselSelector' . $id . '" role="tablist" on="select:carousel' . $id . '.goToSlide(index=event.targetOption)"><amp-carousel id="carouselPreview' . $id . '" class="carousel-preview" height="40" layout="fixed-height" type="carousel">';
            if ($row1) {
                $i = 0;
                foreach ($row1 as $row) {
                    $select = $i == 0 ? ' selected' : '';
                    $tabs .= '<div id="a' . $id . '-tab' . $i . '" class="item-tab" aria-controls="' . $id . $i . 'a" option="' . $i . '" ' . $select . '><span>' . esc_html($row['title']) . '</span></div>';
                    $i++;
                }
                $tabs .= '</amp-carousel></amp-selector><amp-carousel width="400" height="300" layout="responsive" type="slides" on="slideChange:carouselSelector' . $id . '.toggle(index=event.index, value=true),carouselPreview' . $id . '.goToSlide(index=event.index)" id="carousel' . $id . '">';
                $i = 1;
                foreach ($row1 as $row) {
                    $tabs .= '<div class="slide"><amp-img src="' . esc_url($row['img']) . '" layout="fill" class="img-responsive" alt="banner"></amp-img><div class="caption">' . wp_kses_post($row['content']) . '</div></div>';
                    $i++;
                }
                $tabs .= '</amp-carousel></div>';
            }
        }
        return $tabs;
    }
...
function create_shortcode_sdt($args, $content) {
    if (cele_is_amp()) {
        ...
        return '<div class="deail_reg_canhomau"><form method="POST" target="_top" action-xhr="' . esc_url($post_url) . '" class="form-download1 form-content-' . esc_attr($args["id"]) . '" on="submit-error:quote-lb.hide"><div class="deail_reg_canhomau-inner"><div class="deail_reg_canhomau_right"><div class="deail_reg_canhomau_right_title">' . esc_html($args["title"]) . '</div><div class="deail_reg_canhomau_right_form"><div class="deail_reg_canhomau_right_form1"><input type="hidden" name="link" value="' . esc_attr($_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI']) . '"><input aria-label="Mobile" required id="phone_project' . esc_attr($args["id"]) . '" class="form-control" name="Mobile" placeholder="' . __('Your Phone Number', 'master-gf') . '" type="number"><input class="btn_dkcanhomau_duan_input" value="' . esc_attr($args["button"]) . '" type="submit" name="dangky"></div></div></div></div><div submitting><div id="quote-lb" class="overlay1"><div class="loading_ajax"><div class="lds-spinner"><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div></div></div></div><div submit-error><template type="amp-mustache">{{data}}</template></div></form></div>';
    } else {
        return '<div class="deail_reg_canhomau"><form class="form-download1 form-content-' . esc_attr($args["id"]) . '"><div class="deail_reg_canhomau-inner"><div class="deail_reg_canhomau_right"><div class="deail_reg_canhomau_right_title">' . esc_html($args["title"]) . '</div><div class="deail_reg_canhomau_right_form"><div class="deail_reg_canhomau_right_form1"><input aria-label="Mobile" id="phone_project' . esc_attr($args["id"]) . '" class="form-control" name="Mobile" required="" placeholder="' . __('Your Phone Number', 'master-gf') . '" type="number"><input class="btn_dkcanhomau_duan_input" onclick="form_content(' . esc_attr($args["id"]) . ')" name="dangky" value="' . esc_attr($args["button"]) . '" type="button"></div></div></div></div></form></div>';
    }
}

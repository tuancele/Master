	...
	class toc {
		...
		function __construct()
		{
			...
			$defaults = array(		// default options
				...
				'exclude_css' => false,
				'exclude' => '',
				'heading_levels' => array('6', '5', '4', '3', '2', '1'),
				'restrict_path' => '',
				'css_container_class' => '',
		...
		{
			if ( $file == 'cele-table/' . basename(__FILE__) ) {
				$settings_link = '<a href="options-general.php?page=toc">' . __('Settings', 'cele-table') . '</a>';
		...
		function wp_enqueue_scripts()
		{
			...
			wp_register_style( 'toc-screen', get_stylesheet_directory_uri() . '/inc/cele-table/screen.min.css', array(), TOC_VERSION );
			wp_register_script( 'toc-front', get_stylesheet_directory_uri() . '/inc/cele-table/front.min.js', array('jquery'), TOC_VERSION, true );
		...
		{
			load_plugin_textdomain( 'cele-table', false, dirname(plugin_basename(__FILE__)) . '/languages/' );
		}
		
		
		function admin_init()
		{
			wp_register_script( 'toc_admin_script', get_stylesheet_directory_uri() . '/inc/cele-table/admin.js', array(), '2.9.6' );
			wp_register_style( 'toc_admin_style', get_stylesheet_directory_uri() . '/inc/cele-table/admin.css', array(), '2.9.6' );
		...
		function admin_options() 
		{
			...
			if ( isset( $_GET['update'] ) ) {
				if ( $this->save_admin_options() )
					$msg = '<div id="message" class="updated fade"><p>' . __('Options saved.', 'cele-table') . '</p></div>';
				else
					$msg = '<div id="message" class="error fade"><p>' . __('Save failed.', 'cele-table') . '</p></div>';
			}
...
<ul id="tabbed-nav">
	<li><a href="#tab1"><?php _e('Main Options', 'cele-table'); ?></a></li>
	<li><a href="#tab2"><?php _e('Sitemap', 'cele-table'); ?></a></li>
</ul>
...
<tbody>
<tr>
	<th><label for="position"><?php _e('Position', 'cele-table'); ?></label></th>
	<td>
		<select name="position" id="position">
			<option value="<?php echo TOC_POSITION_BEFORE_FIRST_HEADING; ?>"<?php if ( TOC_POSITION_BEFORE_FIRST_HEADING == $this->options['position'] ) echo ' selected="selected"'; ?>><?php _e('Before first heading (default)', 'cele-table'); ?></option>
			<option value="<?php echo TOC_POSITION_AFTER_FIRST_HEADING; ?>"<?php if ( TOC_POSITION_AFTER_FIRST_HEADING == $this->options['position'] ) echo ' selected="selected"'; ?>><?php _e('After first heading', 'cele-table'); ?></option>
			<option value="<?php echo TOC_POSITION_TOP; ?>"<?php if ( TOC_POSITION_TOP == $this->options['position'] ) echo ' selected="selected"'; ?>><?php _e('Top', 'cele-table'); ?></option>
			<option value="<?php echo TOC_POSITION_BOTTOM; ?>"<?php if ( TOC_POSITION_BOTTOM == $this->options['position'] ) echo ' selected="selected"'; ?>><?php _e('Bottom', 'cele-table'); ?></option>
		</select>
	</td>
</tr>
<tr>
	<th><label for="start"><?php _e('Show when', 'cele-table'); ?></label></th>
	<td>
...
?>
		...
		_e('or more headings are present', 'cele-table'); ?>
	</td>
</tr>
<tr>
	<th><?php _e('Auto insert for the following content types', 'cele-table'); ?></th>
	<td><?php
...
<tr>
	...
	_e('Heading text', 'cele-table'); ?></label></th>
	<td>
		<input type="checkbox" value="1" id="show_heading_text" name="show_heading_text"<?php if ( $this->options['show_heading_text'] ) echo ' checked="checked"'; ?> /><label for="show_heading_text"> <?php _e('Show title on top of the table of contents', 'cele-table'); ?></label><br />
		<div class="more_toc_options<?php if ( !$this->options['show_heading_text'] ) echo ' disabled'; ?>">
			...
			<span class="description"><label for="heading_text"><?php _e('Eg: Contents, Table of Contents, Page Contents', 'cele-table'); ?></label></span><br /><br />
			
			<input type="checkbox" value="1" id="visibility" name="visibility"<?php if ( $this->options['visibility'] ) echo ' checked="checked"'; ?> /><label for="visibility"> <?php _e( 'Allow the user to toggle the visibility of the table of contents', 'cele-table'); ?></label><br />
			<div class="more_toc_options<?php if ( !$this->options['visibility'] ) echo ' disabled'; ?>">
				<table class="more_toc_options_table">
				<tbody>
				<tr>
					<th><label for="visibility_show"><?php _e('Show text', 'cele-table'); ?></label></th>
					...
					_e('Eg: show', 'cele-table'); ?></label></span></td>
				</tr>
				<tr>
					<th><label for="visibility_hide"><?php _e('Hide text', 'cele-table'); ?></label></th>
					...
					_e('Eg: hide', 'cele-table'); ?></label></span></td>
				</tr>
				</tbody>
				</table>
				<input type="checkbox" value="1" id="visibility_hide_by_default" name="visibility_hide_by_default"<?php if ( $this->options['visibility_hide_by_default'] ) echo ' checked="checked"'; ?> /><label for="visibility_hide_by_default"> <?php _e( 'Hide the table of contents initially', 'cele-table'); ?></label>
			</div>
		</div>
	</td>
</tr>
<tr>
	<th><label for="show_heirarchy"><?php _e('Show hierarchy', 'cele-table'); ?></label></th>
...
<tr>
	<th><label for="ordered_list"><?php _e('Number list items', 'cele-table'); ?></label></th>
...
<tr>
	<th><label for="smooth_scroll"><?php _e('Enable smooth scroll effect', 'cele-table'); ?></label></th>
	<td><input type="checkbox" value="1" id="smooth_scroll" name="smooth_scroll"<?php if ( $this->options['smooth_scroll'] ) echo ' checked="checked"'; ?> /><label for="smooth_scroll"> <?php _e( 'Scroll rather than jump to the anchor link', 'cele-table'); ?></label></td>
</tr>
</tbody>
</table>

<h3><?php _e('Appearance', 'cele-table'); ?></h3>
<table class="form-table">
<tbody>
<tr>
	<th><label for="width"><?php _e('Width', 'cele-table'); ?></label></td>
	<td>
		<select name="width" id="width">
			<optgroup label="<?php _e('Fixed width', 'cele-table'); ?>">
			...
			<optgroup label="<?php _e('Relative', 'cele-table'); ?>">
				<option value="Auto"<?php if ( 'Auto' == $this->options['width'] ) echo ' selected="selected"'; ?>><?php _e('Auto (default)', 'cele-table'); ?></option>
			...
			_e('Other', 'cele-table'); ?>">
				<option value="User defined"<?php if ( 'User defined' == $this->options['width'] ) echo ' selected="selected"'; ?>><?php _e('User defined', 'cele-table'); ?></option>
			</optgroup>
		</select>
		<div class="more_toc_options<?php if ( 'User defined' != $this->options['width'] ) echo ' disabled'; ?>">
			...
			printf( __('Please enter a number and %s select its units, eg: 100px, 10em', 'cele-table'), '</label><label for="width_custom_units">' ); ?></label><br />
...
<tr>
	<th><label for="wrapping"><?php _e('Wrapping', 'cele-table'); ?></label></td>
	<td>
		<select name="wrapping" id="wrapping">
			<option value="<?php echo TOC_WRAPPING_NONE; ?>"<?php if ( TOC_WRAPPING_NONE == $this->options['wrapping'] ) echo ' selected="selected"'; ?>><?php _e('None (default)', 'cele-table'); ?></option>
			<option value="<?php echo TOC_WRAPPING_LEFT; ?>"<?php if ( TOC_WRAPPING_LEFT == $this->options['wrapping'] ) echo ' selected="selected"'; ?>><?php _e('Left', 'cele-table'); ?></option>
			<option value="<?php echo TOC_WRAPPING_RIGHT; ?>"<?php if ( TOC_WRAPPING_RIGHT == $this->options['wrapping'] ) echo ' selected="selected"'; ?>><?php _e('Right', 'cele-table'); ?></option>
		</select>
	</td>
</tr>
<tr>
	<th><label for="font_size"><?php _e('Font size', 'cele-table'); ?></label></th>
	<td>
...
<tr>
	...
	_e('Presentation', 'cele-table'); ?></th>
	<td>
		<div class="toc_theme_option">
			<input type="radio" name="theme" id="theme_<?php echo TOC_THEME_GREY; ?>" value="<?php echo TOC_THEME_GREY; ?>"<?php if ( $this->options['theme'] == TOC_THEME_GREY ) echo ' checked="checked"'; ?> /><label for="theme_<?php echo TOC_THEME_GREY; ?>"> <?php _e('Grey (default)', 'cele-table'); ?><br />
			<img src="<?php bloginfo('template_url'); ?>/inc/cele-table/images/grey.png" alt="" />
			</label>
		</div>
		<div class="toc_theme_option">
			<input type="radio" name="theme" id="theme_<?php echo TOC_THEME_LIGHT_BLUE; ?>" value="<?php echo TOC_THEME_LIGHT_BLUE; ?>"<?php if ( $this->options['theme'] == TOC_THEME_LIGHT_BLUE ) echo ' checked="checked"'; ?> /><label for="theme_<?php echo TOC_THEME_LIGHT_BLUE; ?>"> <?php _e('Light blue', 'cele-table'); ?><br />
			<img src="<?php bloginfo('template_url'); ?>/inc/cele-table/images/blue.png" alt="" />
			</label>
		</div>
		<div class="toc_theme_option">
			<input type="radio" name="theme" id="theme_<?php echo TOC_THEME_WHITE; ?>" value="<?php echo TOC_THEME_WHITE; ?>"<?php if ( $this->options['theme'] == TOC_THEME_WHITE ) echo ' checked="checked"'; ?> /><label for="theme_<?php echo TOC_THEME_WHITE; ?>"> <?php _e('White', 'cele-table'); ?><br />
			<img src="<?php bloginfo('template_url'); ?>/inc/cele-table/images/white.png" alt="" />
			</label>
		</div>
		<div class="toc_theme_option">
			<input type="radio" name="theme" id="theme_<?php echo TOC_THEME_BLACK; ?>" value="<?php echo TOC_THEME_BLACK; ?>"<?php if ( $this->options['theme'] == TOC_THEME_BLACK ) echo ' checked="checked"'; ?> /><label for="theme_<?php echo TOC_THEME_BLACK; ?>"> <?php _e('Black', 'cele-table'); ?><br />
			<img src="<?php bloginfo('template_url'); ?>/inc/cele-table/images/black.png" alt="" />
			</label>
		</div>
		<div class="toc_theme_option">
			<input type="radio" name="theme" id="theme_<?php echo TOC_THEME_TRANSPARENT; ?>" value="<?php echo TOC_THEME_TRANSPARENT; ?>"<?php if ( $this->options['theme'] == TOC_THEME_TRANSPARENT ) echo ' checked="checked"'; ?> /><label for="theme_<?php echo TOC_THEME_TRANSPARENT; ?>"> <?php _e('Transparent', 'cele-table'); ?><br />
			<img src="<?php bloginfo('template_url'); ?>/inc/cele-table/images/transparent.png" alt="" />
			</label>
		</div>
		<div class="toc_theme_option">
			<input type="radio" name="theme" id="theme_<?php echo TOC_THEME_CUSTOM; ?>" value="<?php echo TOC_THEME_CUSTOM; ?>"<?php if ( $this->options['theme'] == TOC_THEME_CUSTOM ) echo ' checked="checked"'; ?> /><label for="theme_<?php echo TOC_THEME_CUSTOM; ?>"> <?php _e('Custom', 'cele-table'); ?><br />
			<img src="<?php bloginfo('template_url'); ?>/inc/cele-table/images/custom.png" alt="" />
			</label>
		</div>
		<div class="clear"></div>
		...
		<div class="more_toc_options<?php if ( TOC_THEME_CUSTOM != $this->options['theme'] ) echo ' disabled'; ?>">
			...
			<tbody>
			<tr>
				<th><label for="custom_background_colour"><?php _e('Background', 'cele-table'); ?></label></th>
				<td><input type="text" class="custom_colour_option" value="<?php echo htmlentities( $this->options['custom_background_colour'] ); ?>" id="custom_background_colour" name="custom_background_colour" /> <img src="<?php bloginfo('template_url'); ?>/inc/cele-table/images/colour-wheel.png" alt="" /></td>
			</tr>
			<tr>
				<th><label for="custom_border_colour"><?php _e('Border', 'cele-table'); ?></label></th>
				<td><input type="text" class="custom_colour_option" value="<?php echo htmlentities( $this->options['custom_border_colour'] ); ?>" id="custom_border_colour" name="custom_border_colour" /> <img src="<?php bloginfo('template_url'); ?>/inc/cele-table/images/colour-wheel.png" alt="" /></td>
			</tr>
			<tr>
				<th><label for="custom_title_colour"><?php _e('Title', 'cele-table'); ?></label></th>
				<td><input type="text" class="custom_colour_option" value="<?php echo htmlentities( $this->options['custom_title_colour'] ); ?>" id="custom_title_colour" name="custom_title_colour" /> <img src="<?php bloginfo('template_url'); ?>/inc/cele-table/images/colour-wheel.png" alt="" /></td>
			</tr>
			<tr>
				<th><label for="custom_links_colour"><?php _e('Links', 'cele-table'); ?></label></th>
				<td><input type="text" class="custom_colour_option" value="<?php echo htmlentities( $this->options['custom_links_colour'] ); ?>" id="custom_links_colour" name="custom_links_colour" /> <img src="<?php bloginfo('template_url'); ?>/inc/cele-table/images/colour-wheel.png" alt="" /></td>
			</tr>
			<tr>
				<th><label for="custom_links_hover_colour"><?php _e('Links (hover)', 'cele-table'); ?></label></th>
				<td><input type="text" class="custom_colour_option" value="<?php echo htmlentities( $this->options['custom_links_hover_colour'] ); ?>" id="custom_links_hover_colour" name="custom_links_hover_colour" /> <img src="<?php bloginfo('template_url'); ?>/inc/cele-table/images/colour-wheel.png" alt="" /></td>
			</tr>
			<tr>
				<th><label for="custom_links_visited_colour"><?php _e('Links (visited)', 'cele-table'); ?></label></th>
				<td><input type="text" class="custom_colour_option" value="<?php echo htmlentities( $this->options['custom_links_visited_colour'] ); ?>" id="custom_links_visited_colour" name="custom_links_visited_colour" /> <img src="<?php bloginfo('template_url'); ?>/inc/cele-table/images/colour-wheel.png" alt="" /></td>
			</tr>
			</tbody>
			</table>
			<div id="farbtastic_colour_wheel"></div>
			<div class="clear"></div>
			<p><?php printf(__("Leaving the value as %s will inherit your theme's styles", 'cele-table'), '<code>#</code>'); ?></p>
		</div>
	</td>
</tr>
</tbody>
...
<h3><?php _e('Advanced', 'cele-table'); ?> <span class="show_hide">(<a href="#toc_advanced_usage"><?php _e('show', 'cele-table'); ?></a>)</span></h3>
<div id="toc_advanced_usage">
	<h4><?php _e('Power options', 'cele-table'); ?></h4>
	<table class="form-table">
	<tbody>
	<tr>
		<th><label for="lowercase"><?php _e('Lowercase', 'cele-table'); ?></label></th>
		<td><input type="checkbox" value="1" id="lowercase" name="lowercase"<?php if ( $this->options['lowercase'] ) echo ' checked="checked"'; ?> /><label for="lowercase"> <?php _e('Ensure anchors are in lowercase', 'cele-table'); ?></label></td>
	</tr>
	<tr>
		<th><label for="hyphenate"><?php _e('Hyphenate', 'cele-table'); ?></label></th>
		<td><input type="checkbox" value="1" id="hyphenate" name="hyphenate"<?php if ( $this->options['hyphenate'] ) echo ' checked="checked"'; ?> /><label for="hyphenate"> <?php _e('Use - rather than _ in anchors', 'cele-table'); ?></label></td>
	</tr>
	<tr>
		<th><label for="include_homepage"><?php _e('Include homepage', 'cele-table'); ?></label></th>
		<td><input type="checkbox" value="1" id="include_homepage" name="include_homepage"<?php if ( $this->options['include_homepage'] ) echo ' checked="checked"'; ?> /><label for="include_homepage"> <?php _e('Show the table of contents for qualifying items on the homepage', 'cele-table'); ?></label></td>
	</tr>
	<tr>
		<th><label for="exclude_css"><?php _e('Exclude CSS file', 'cele-table'); ?></label></th>
		<td><input type="checkbox" value="1" id="exclude_css" name="exclude_css"<?php if ( $this->options['exclude_css'] ) echo ' checked="checked"'; ?> /><label for="exclude_css"> <?php _e("Prevent the loading of this plugin's CSS styles. When selected, the appearance options from above will also be ignored.", 'cele-table'); ?></label></td>
	</tr>
	<tr>
		<th><label for="bullet_spacing"><?php _e('Preserve theme bullets', 'cele-table'); ?></label></th>
		<td><input type="checkbox" value="1" id="bullet_spacing" name="bullet_spacing"<?php if ( $this->options['bullet_spacing'] ) echo ' checked="checked"'; ?> /><label for="bullet_spacing"> <?php _e('If your theme includes background images for unordered list elements, enable this to support them', 'cele-table'); ?></label></td>
	</tr>
	<tr>
		<th><?php _e('Heading levels', 'cele-table'); ?></th>
		<td>
		<p><?php _e('Include the following heading levels. Deselecting a heading will exclude it.', 'cele-table'); ?></p>
<?php
...
?>
	...
	<tr>
		<th><label for="exclude"><?php _e('Exclude headings', 'cele-table'); ?></label></th>
		<td>
			...
			<label for="exclude"><?php _e('Specify headings to be excluded from appearing in the table of contents.  Separate multiple headings with a pipe <code>|</code>.  Use an asterisk <code>*</code> as a wildcard to match other text.  Note that this is not case sensitive. Some examples:', 'cele-table'); ?></label><br/>
			<ul>
				<li><?php _e('<code>Fruit*</code> ignore headings starting with "Fruit"', 'cele-table'); ?></li>
				<li><?php _e('<code>*Fruit Diet*</code> ignore headings with "Fruit Diet" somewhere in the heading', 'cele-table'); ?></li>
				<li><?php _e('<code>Apple Tree|Oranges|Yellow Bananas</code> ignore headings that are exactly "Apple Tree", "Oranges" or "Yellow Bananas"', 'cele-table'); ?></li>
			</ul>
		</td>
	</tr>
	<tr id="smooth_scroll_offset_tr" class="<?php if ( !$this->options['smooth_scroll'] ) echo 'disabled'; ?>">
		<th><label for="smooth_scroll_offset"><?php _e('Smooth scroll top offset', 'cele-table'); ?></label></th>
		<td>
			...
			<label for="smooth_scroll_offset"><?php _e('If you have a consistent menu across the top of your site, you can adjust the top offset to stop the headings from appearing underneath the top menu. A setting of 30 accommodates the WordPress admin bar. This setting appears after you have enabled smooth scrolling from above.', 'cele-table'); ?></label>
		</td>
	</tr>
	<tr>
		<th><label for="restrict_path"><?php _e('Restrict path', 'cele-table'); ?></label></th>
		<td>
			...
			<label for="restrict_path"><?php _e('Restrict generation of the table of contents to pages that match the required path. This path is from the root of your site and always begins with a forward slash.', 'cele-table'); ?><br />
			<span class="description"><?php 
			/* translators: example URL path restriction */
			_e('Eg: /wiki/, /corporate/annual-reports/', 'cele-table'); ?></span></label>
		</td>
	</tr>
	<tr>
		<th><label for="fragment_prefix"><?php _e('Default anchor prefix', 'cele-table'); ?></label></th>
		<td>
			...
			<label for="fragment_prefix"><?php _e('Anchor targets are restricted to alphanumeric characters as per HTML specification (see readme for more detail). The default anchor prefix will be used when no characters qualify. When left blank, a number will be used instead.', 'cele-table'); ?><br />
			<?php _e('This option normally applies to content written in character sets other than ASCII.', 'cele-table'); ?><br />
			<span class="description"><?php 
			/* translators: example anchor prefixes when no ascii characters match */
			_e('Eg: i, toc_index, index, _', 'cele-table'); ?></span></label>
		</td>
	</tr>
	</tbody>
	</table>
	...
	_e('Usage', 'cele-table'); ?></h4>
	<p><?php printf(__('If you would like to fully customise the position of the table of contents, you can use the %s shortcode by placing it at the desired position of your post, page or custom post type. This method allows you to generate the table of contents despite having auto insertion disabled for its content type. Please visit the help tab for further information about this shortcode.', 'cele-table'), '<code>[toc]</code>'); ?></p>
</div>
...
<p><?php printf(__('At its simplest, placing %s into a page will automatically create a sitemap of all pages and categories. This also works in a text widget.', 'cele-table'), '<code>[sitemap]</code>'); ?></p>
<table class="form-table">
<tbody>
<tr>
	<th><label for="sitemap_show_page_listing"><?php _e('Show page listing', 'cele-table'); ?></label></th>
...
<tr>
	<th><label for="sitemap_show_category_listing"><?php _e('Show category listing', 'cele-table'); ?></label></th>
...
<tr>
	<th><label for="sitemap_heading_type"><?php _e('Heading type', 'cele-table'); ?></label></th>
	...
	_e('Use', 'cele-table'); ?> h</label><select name="sitemap_heading_type" id="sitemap_heading_type">
<?php
			// h1 to h6
			for ($i = 1; $i <= 6; $i++) {
				echo '<option value="' . $i . '"';
...
?>
		...
		_e('to print out the titles', 'cele-table'); ?>
	</td>
</tr>
<tr>
	<th><label for="sitemap_pages"><?php _e('Pages label', 'cele-table'); ?></label></th>
	<td><input type="text" class="regular-text" value="<?php echo htmlentities( $this->options['sitemap_pages'], ENT_COMPAT, 'UTF-8' ); ?>" id="sitemap_pages" name="sitemap_pages" />
		<span class="description"><?php _e('Eg: Pages, Page List', 'cele-table'); ?></span>
	</td>
</tr>
<tr>
	<th><label for="sitemap_categories"><?php _e('Categories label', 'cele-table'); ?></label></th>
	<td><input type="text" class="regular-text" value="<?php echo htmlentities( $this->options['sitemap_categories'], ENT_COMPAT, 'UTF-8' ); ?>" id="sitemap_categories" name="sitemap_categories" />
		<span class="description"><?php _e('Eg: Categories, Category List', 'cele-table'); ?></span>
	</td>
</tr>
</tbody>
</table>

<h3><?php _e('Advanced usage', 'cele-table'); ?> <span class="show_hide">(<a href="#sitemap_advanced_usage"><?php _e('show', 'cele-table'); ?></a>)</span></h3>
<div id="sitemap_advanced_usage">
	<p><code>[sitemap_pages]</code> <?php printf(__('lets you print out a listing of only pages. Similarly %s can be used to print out a category listing. They both can accept a number of attributes so visit the help tab for more information.', 'cele-table'), '<code>[sitemap_categories]</code>'); ?></p>
	<p><?php _e('Examples', 'cele-table'); ?></p>
	<ol>
		<li><code>[sitemap_categories no_label="true"]</code> <?php _e('hides the heading from a category listing', 'cele-table'); ?></li>
		<li><code>[sitemap_pages heading="6" label="This is an awesome listing" exclude="1,15"]</code> <?php printf(__('Uses h6 to display %s on a page listing excluding pages with IDs 1 and 15', 'cele-table'), '<em>This is an awesome listing</em>'); ?></li>
	</ol>
</div>
...
<p class="submit"><input type="submit" name="submit" class="button-primary" value="<?php _e('Update Options', 'cele-table'); ?>" /></p>
</form>
</div>
<?php
		}
	...
	class toc_widget extends WP_Widget {
		...
		function __construct()
		{
			$widget_options = array( 
				...
				'description' => __('Display the table of contents in the sidebar with this widget', 'cele-table') 
			);
			...
			parent::__construct( 'toc-widget', 'Bảng nội dung', $widget_options, $control_options );
		}
...
?>
			<p>
				<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e('Title', 'cele-table'); ?>:</label>
			...
			<p>
				...
				<label for="<?php echo $this->get_field_id( 'hide_inline' ); ?>"> <?php _e('Show the table of contents only in the sidebar', 'cele-table'); ?></label>
			</p>
			...
			<div class="show_toc_in_widget_only_post_types" style="margin: 0 0 25px 25px; display: <?php echo ( $toc_options['show_toc_in_widget_only'] == 1 ) ? 'block' : 'none'; ?>;">
				<p><?php _e('For the following content types:', 'cele-table'); ?></p>
